        ...
        try:
            async for message in websocket:
                try:
                    ...
                    logger.info(
                        f"Received message from client {websocket.remote_address}: {data}")
                    ...
                    if command == "getData":
                        ...
                        logger.info(
                            f"Sending data to client {websocket.remote_address}, response: {json.dumps(response)}")
                        ...
                        logger.info(
                            f"Sending fan data to client {websocket.remote_address}, response: {json.dumps(response)}")
                        await websocket.send(json.dumps(response))
                        ...
                        logger.info(
                            f"Sending heater data to client {websocket.remote_address}, response: {json.dumps(response)}")
                        await websocket.send(json.dumps(response))
                    elif command == "fanUp":
                        ...
                        try:
                            ...
                            response = {
                                "id": req_id, "status": "success" if success else "error"}
                            await websocket.send(json.dumps(response))
