...
class Machine:
    ...
    async def discover_characteristics(self, client):
        ...
        for service in client.services:
            ...
            for char in service.characteristics:
                ...
                self.logger.info(f"  [Characteristic] {char.uuid} | Properties: {props}")
                if "notify" in char.properties and not self.notify_characteristic_uuid:
                    self.notify_characteristic_uuid = char.uuid
                    self.logger.info(f"    -> Found characteristic for notifications: {self.notify_characteristic_uuid}")
                if "write" in char.properties and not self.write_characteristic_uuid:
                    self.write_characteristic_uuid = char.uuid
                    self.logger.info(f"    -> Found characteristic for writing: {self.write_characteristic_uuid}")
        
        if not self.notify_characteristic_uuid:
            self.logger.error("Could not find a characteristic with 'notify' property.")
            return False
        if not self.write_characteristic_uuid:
            self.logger.error("Could not find a characteristic with 'write' property.")
            return False
        return True
    ...
    async def subscribe_to_notifications(self, client, callback):
        ...
        self.logger.info(f"Subscribing to notifications for characteristic {self.notify_characteristic_uuid}")
    ...
    async def unsubscribe_from_notifications(self, client):
        if client.is_connected and self.notify_characteristic_uuid:
            try:
                self.logger.info(f"Unsubscribing from notifications for {self.notify_characteristic_uuid}")
    ...
    async def send_command(self, client, command):
        ...
        async with self.command_lock:
            try:
                ...
                self.logger.info(f"Sending command: {command} (byte length: {len(command_bytes)})")
                
                if not client.is_connected:
                    self.logger.error("Cannot send command: BLE client is disconnected")
                    return False
    ...
    async def set_fan(self, client, value):
        if value < 0 or value > 100:
            self.logger.error(f"Invalid fan value: {value}. Must be between 0 and 100.")
            return False
    ...
    async def set_heater(self, client, value):
        if value < 0 or value > 100:
            self.logger.error(f"Invalid heater value: {value}. Must be between 0 and 100.")
            return False
